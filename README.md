...
The backend (`backend/`) is not wired into these pages yet — the frontend still runs entirely on the realistic mock data under `src/lib/data/`. The login screen includes shortcuts to preview both dashboards directly. See `backend/README.md` for the current state of the API: authentication, user management, role/permission authorization, multi-tenant gym/business management, a Members/Leads-CRM domain (member roster, lead lifecycle, trial scheduling, and lead-to-member conversion), Trainers, Classes/Scheduling, Class Bookings/Waitlist, and Personal Training (recurring class templates with lazily generated occurrences, capacity-aware booking with concurrency-safe FIFO waitlisting, trainer-availability-driven PT booking), Membership Plans and Member Memberships (plan management, date-derived lifecycle with no expiry cron needed, renewal with plan switching, freeze/unfreeze, cancellation, full per-member history via a renewal chain), Payments/Billing, Transactions, Invoices, and Refunds (one reusable transaction model for every charge type, auto-generated invoices with safe sequential numbering, full/partial refunds with concurrency-safe validation, membership-purchase/renewal payment linkage), and now Notifications & Communication (an event-driven in-app notification system covering booking/membership/payment lifecycle events, per-user preferences, gym-wide announcements, and scheduled reminders) are implemented and tested, but Attendance doesn't exist yet, so there's nothing to point most of this frontend at until later phases.
